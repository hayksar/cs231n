...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if cell_type == 'rnn':
            ...
            h, cache_rnn = lstm_forward(out_word, h0, Wx, Wh, b)
        out_aff, cache_aff = temporal_affine_forward(h, W_vocab, b_vocab)
        loss, dout_aff = temporal_softmax_loss(out_aff, captions_out, mask)
        
        dh, dW_vocab, db_vocab = temporal_affine_backward(dout_aff, cache_aff)
        if (cell_type == 'rnn'):
            dout_word, dh0, dWx, dWh, db = rnn_backward(dh, cache_rnn)
        else:
            dout_word, dh0, dWx, dWh, db = lstm_backward(dh, cache_rnn)
