...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        xmean = x - mean
        sq = xmean**2
        std = 1./N * np.sum(sq, axis = 0)
        sqrtstd = np.sqrt(std + eps)
        istd = 1./sqrtstd
        x_norm = xmean * istd
        gammax = gamma * x_norm
        out = gammax + beta
        running_mean = momentum * running_mean + (1 - momentum) * mean
        running_var = momentum * running_var + (1 - momentum) * np.sqrt(std)
        cache = (x_norm, gamma, xmean, istd, sqrtstd, std, eps)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    x_norm, gamma, xmean, istd, sqrtstd, std, eps = cache
    N, D = dout.shape
    ###########################################################################
    ...
    dx_norm = dout * gamma
    distd = np.sum(dx_norm * xmean, axis=0)
    dxmean1 = dx_norm * istd
    dsqrtstd = -1. / (sqrtstd**2) * distd
    dstd = 0.5 * 1. / np.sqrt(std + eps) * dsqrtstd
    dsq = 1. / N * np.ones((N,D)) * dstd
    dxmean2 = 2 * xmean * dsq
    dx1 = (dxmean1 + dxmean2)
    dmean = -1 * np.sum(dxmean1 + dxmean2, axis=0)
    dx2 = 1. / N * np.ones((N,D)) * dmean
    dx = dx1 + dx2
    #dstd = -0.5 * np.sum(dout * (x - mean) * np.power(std**2 + eps, -1.5), axis = 0)
    #dmean = -np.sum(dout / np.sqrt(std**2 + eps), axis = 0) - 2 * dstd * np.mean(x - mean, axis = 0)
    #dx = dout / np.sqrt(std**2 + eps) + 2 * dstd / N * (x - mean) + dmean / N
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    for n in range(0, N):
        ...
        for i in range(0, out_h):
            for j in range(0, out_w):
                x_f = x_data[:, (i * stride):(i * stride + HH), (j * stride):(j * stride + WW)]
                for f in range(0, F):
                    out[n, f, i, j] = np.sum(x_f * w[f]) + b[f]
    ...
    for n in range(0, N):
        ...
        for f in range(0, F):
            for i in range(0, out_h):
                for j in range(0, out_w):
                    x_f = x_data[:, (i * stride):(i * stride + HH), (j * stride):(j * stride + WW)]
                    dw[f] += x_f * dout[n, f, i, j]
                    dx_data[:, (i * stride):(i * stride + HH), (j * stride):(j * stride + WW)] += w[f] * dout[n, f, i, j]
        dx[n,:,:,:] = dx_data[:,1:-1,1:-1]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw, db
        ...
        for c in range(C):
            for h in range(out_h):
                for w in range(out_w):
                    out[n, c, h, w] = np.max(x[n, c, stride * h:stride*h+pool_height, stride * w:stride*w+pool_width])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in xrange(N):
        for c in xrange(C):
            for h in xrange(hprime):
                for w in xrange(wprime):
                  h1 = h * stride
